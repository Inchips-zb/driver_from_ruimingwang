...
struct ring_buf *ring_buf_init(void *buf, int total_size, void (*highwater_cb)(struct ring_buf *buf), uint32_t free_size_low, uint32_t free_size_high)
{
    struct ring_buf *r = (struct ring_buf *)buf;
    r->total_size = total_size - sizeof(struct ring_buf) + 1;
    r->free_size_low = free_size_low;
    r->free_size_high = free_size_high;
...
{
    if (free_size < buf->free_size_low)
    {
